...
PUBLIC        void      Decoder::Create(MediaSource* src, int stream_index, int out_b_size, void (*freeOutFunc)(void*), int thread_count) {
    assert(out_b_size > 0);
    ...
    for (int i = 0; i < KIT_DEC_BUF_COUNT; i++) {
        Buffer[i] = new class Buffer(bsizes[i], free_hooks[i]);
        if (Buffer[i] == NULL) {
...
PUBLIC        int       Decoder::GetCodecInfo(Codec* codec) {
    ...
    snprintf(codec->Name, KIT_CODEC_NAME_MAX, "%s", CodecCtx->codec->name);
    snprintf(codec->Description, KIT_CODEC_DESC_MAX, "%s", CodecCtx->codec->long_name);
    return 0;
}
